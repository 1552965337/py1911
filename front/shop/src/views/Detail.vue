<template>
	...
	<vueToTop :type='2' :size='30' :bottom='50'style="z-index: 10;"></vueToTop>
		...
		<div style="color:red;font-size: 27px;">{{data.Cpmc}}</div>
		<div style="background-color: #F0C674;margin: 10px;">
			<div style="text-align: left; margin-left: 10px;">
				<p><b>材料：{{data.Instro}}</b></p>
				<p><b>配送范围：</b>全国</p>
			</div>
			<div style="text-align: left; margin-left: 10px;">
				<p><b>原价：{{data.LinePrice}}元</b><del></del> </p>
...
<script>
	...
	export default {
	  ...
	  methods: {
		...
		getdata(){
			dictA.forEach(item=>{
					// console.log(item.ItemCode,this.$route.params.id);
					if(item.ItemCode==this.$route.params.id){
